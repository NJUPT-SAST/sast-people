...
export const get_user_access_token = async (
  ...
  try {
    ...
    if (!access_token) {
      return { success: false, error: "get access token failed" };
    }
    return { success: true, data: access_token };
  } catch (error) {
    return {
      success: false,
      error: error instanceof Error ? error.message : "get access token failed"
    };
  }
};
...
export async function bindingLinkAccount(studentId: string): Promise<ActionResult> {
  try {
    ...
    return {
      success: false,
      error: error instanceof Error ? error.message : "Failed to bind link account"
    };
  }
}
